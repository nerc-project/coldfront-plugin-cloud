...
def get_kc_allocator(allocation):
    resource = allocation.resources.first()
    return keycloak.KeyCloakResourceAllocator(resource, allocation)
...
def add_user_to_keycloak(allocation_user_pk):
    allocation_user = AllocationUser.objects.get(pk=allocation_user_pk)
    allocation = allocation_user.allocation
    kc_allocator = get_kc_allocator(allocation)
    username = allocation_user.user.username
    try:
        kc_allocator.assign_role_on_user(username, kc_allocator.group_name)
    except keycloak.MissingKeycloakTemplateError as e:
        logger.info(f"Skipping adding user to Keycloak: {e}")
...
def remove_user_from_keycloak(allocation_user_pk):
    ...
    username = allocation_user.user.username
    try:
        kc_allocator.remove_role_from_user(username, kc_allocator.group_name)
    except keycloak.MissingKeycloakTemplateError as e:
        logger.info(f"Skipping removing user from Keycloak: {e}")

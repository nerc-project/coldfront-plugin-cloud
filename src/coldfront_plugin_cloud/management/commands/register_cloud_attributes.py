...
ALLOCATION_ATTRIBUTE_MIGRATIONS = [
    ...
    (
        ...
        {"name": "OpenShift Limit on NESE Ephemeral Storage Quota (GiB)"},
    ),
    (
    ...
    ("OpenStack Swift Quota in Gigabytes", {"name": "OpenStack Swift Quota (GiB)"}),
]
    ...
    def migrate_allocation_attributes(self):
        for old_name, new_dict in ALLOCATION_ATTRIBUTE_MIGRATIONS:
            ...
            try:
                ...
                attr.save()
                logger.info(
                    f'Allocation attribute "{old_name}" migrated to "{new_dict}".'
                )
                ...
                logger.error(
                    f'Multiple allocation attributes with name "{old_name}".'
    ...
    def migrate_resource_attributes(self):
        for old_name, new_name in RESOURCE_ATTRIBUTE_MIGRATIONS:
            ...
            try:
                ...
                attr.save()
                logger.info(
                    f'Resource attribute "{old_name}" migrated to "{new_name}".'
                )
                ...
                logger.error(
                    f'Multiple resource attributes with name "{old_name}".'
                    f" Cannot perform automatic migration."
                )

    def register_allocation_attributes(self):
        alloc_attrs = (
            attributes.ALLOCATION_ATTRIBUTES + attributes.ALLOCATION_QUOTA_ATTRIBUTES
        )

...
class OpenShiftVMResourceAllocator(openshift.OpenShiftResourceAllocator):
    QUOTA_KEY_MAPPING = {
        ...
        attributes.QUOTA_REQUESTS_VM_GPU_A100_SXM4: lambda x: {"requests.nvidia.com/A100_SXM4_40GB": f"{x}"},
        attributes.QUOTA_REQUESTS_VM_GPU_V100: lambda x: {"requests.nvidia.com/GV100GL_Tesla_V100": f"{x}"},
        attributes.QUOTA_REQUESTS_VM_GPU_H100: lambda x: {"requests.nvidia.com/H100_SXM5_80GB": f"{x}"},

...
from coldfront_plugin_cloud import attributes, openshift_vm, tasks, utils
...
class TestAllocation(base.TestBase):
    ...
    def test_new_allocation(self):
        ...
        allocator = openshift_vm.OpenShiftVMResourceAllocator(self.resource,
                                                         allocation)
        ...
        self.assertEqual(allocation.get_attribute(attributes.QUOTA_REQUESTS_VM_GPU_A100_SXM4), 2 * 0)
        self.assertEqual(allocation.get_attribute(attributes.QUOTA_REQUESTS_VM_GPU_V100), 2 * 0)
        self.assertEqual(allocation.get_attribute(attributes.QUOTA_REQUESTS_VM_GPU_H100), 2 * 0)
